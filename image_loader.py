...
class ImageLoader(data.Dataset):
  ...
  def load_imagepaths_with_labels(self,
    ...
    # go through all files in data directory 
    for curr in os.listdir(self.curr_folder):
      # current file
      curr = os.path.join(self.curr_folder, curr)
      # get value of class index from dictionary based on name after final "/"
      class_ind = curr.split("/")[-1]
    
      for i in os.listdir(curr):
          i = os.path.join(curr, i)
          
          # append path to current field and class index 
          img_paths.append((i, class_labels[class_ind]))
    ############################################################################
    # Student code end
    ############################################################################
  ...
  def get_classes(self) -> Dict[str, int]:
    ...
    for i in os.listdir(self.curr_folder):
      # Assign integer index to the class 
      ind += 1
      classes[str(i)] = ind
    ############################################################################
    # Student code end
  ...
  def __getitem__(self, index: int) -> Tuple[torch.tensor, int]:
    ...
    # Student code start
    ############################################################################
    # get info from self.dataset
    i = self.dataset[index]
    
    class_idx = i[1]
    # load image from path in dataset
    img = self.load_img_from_path(i[0])

    # Apply transforms if valid
    if self.transform:
